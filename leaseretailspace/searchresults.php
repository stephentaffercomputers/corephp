...
<link rel="stylesheet" href="https://leaseretailspace.net/js/leaflet/leaflet.css" />
<!--[if lte IE 8]>
<link rel="stylesheet" href="https://leaseretailspace.net/leaflet/leaflet.ie.css" />
<![endif]-->
<link rel="stylesheet" href="https://leaseretailspace.net/js/leaflet/MarkerCluster.Default.css" />
<!--[if lte IE 8]>
<link rel="stylesheet" href="https://leaseretailspace.net/leaflet/MarkerCluster.Default.ie.css" />
<![endif]-->
<style type="text/css" title="currentStyle">
    @import "https://leaseretailspace.net/js/datatables/css/demo_table.css";
...
if (sizeof($q) > 1 ) $search_sql_stmt = "select distinct *, 1 AS rank from retail_listing where ListingIsActive = 'y' AND CityName ='".trim($q[0])."' and PostalCode = '".trim($q[1])."'";
else {
	if ($q[0] == 'Arlington' || $q[0] == 'arlington') $search_sql_stmt = "select distinct *, 1 AS rank from retail_listing WHERE ListingIsActive = 'y' AND CityName = '".$q[0]."'";
	else 
		if (strlen($q[0]) == 2) $search_sql_stmt = "select distinct *, 1 AS rank from retail_listing WHERE ListingIsActive = 'y' AND StateProvCode like '%".$q[0]."%'";
		else
		{
		//	 $search_sql_stmt = "select distinct * from retail_listing WHERE ListingIsActive = 'y' AND concat(CityName, StateProvCode, PostalCode) like '%".$q[0]."%'";
 				if(!empty($states[$q[0]]))
				{
					$search = $states[$q[0]];
			   $search_sql_stmt = "select distinct *, 1 AS rank from retail_listing WHERE ListingIsActive = 'y' AND  StateProvCode ='". $search."'";
				}
				else
				{
					$search = $q[0];
			  $search_sql_stmt = "select distinct *, 1 AS rank from retail_listing WHERE ListingIsActive = 'y' AND concat(CityName, StateProvCode, PostalCode) like '%".trim(str_replace("-", " ", $search))."%'";
				}
	...
	if ($rec_count > 0)
	{
		...
		while ($row = mysqli_fetch_array($result_page)) {
        	...
        	$sidebar_content .= '<div class="col-sm-4"><div class="shop-item"><div class="image">
								<a href="/retail-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'"><img class="img-polaroid" id="img'.$row['ListingID'].'" height="150" alt="'.$row['StreetAddress'].'" src="';
			...
			$sidebar_content .='"></a>
							...
							<!-- Product Title -->
							<div class="title">
								<h3><a href="/retail-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'">'.$mod_StreetAddress.'</a></h3><center><small>'.$row['CityName'].', '.$row["StateProvCode"].'</small></center>
							</div>
							<div class="price">';
...
if ($row['RentalRateMin'] != "Negotiable") $sidebar_content .=  '<br /><span style="font-weight: normal; font-size: 14px;">('.$rental_rate_min.')</span>';
			...
			$sidebar_content .= '</div><div class="description">
							...
							<div class="actions">
								<a href="/retail-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'" class="btn">More Info</a>
							</div>
						</div></div>';
		}
...
";
				...
				" var contentTxt = '<div><div class=\"row-fluid\"><a href=\"/retail-for-rent/United-States/".$row['StateProvCode']."/".$row['CityName']."/".$row['ListingID']."\">".preg_replace($patterns, '', $row['StreetAddress'].", ".$row['CityName'])."</a><br />".$row['SpaceAvailableTotal']." sq. ft.</div></div>';".
	...
	else
	{
		...
		$firstLatLngNew = "[".$onerow['Longitude'].", ".$onerow['Latitude']."]";
//		$sidebar_content .= "<strong>Your search did not match any properties.</strong><br /><br /><a href=\"https://leaseretailspace.net\">Start a New Search</a>"; 
		$sidebar_content .= "<strong>We are refreshing inventory in ".$_GET["q"].".</strong><br /><br /><a href=\"https://leaseretailspace.net\">Start a New Search</a>"; 
		$cityname = $q[0];
	}
}
...
function list_prop_types(){
	$sql_list_prop_types = "select distinct PropertyType, PropertySubType from retail_listing where ListingIsActive = 'y' order by PropertyType, PropertySubType;";
...
-->
		...
		<div class="section section-breadcrumbs">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h1><a href="/">Home</a> / <? if ($q[0] != $cityname) echo $statecode; else echo "<a href=\"/retail-for-rent/United-States/".$statecode."\">".$statecode."</a> / ".$cityname; ?> Retail Spaces for Rent</h1>
					</div>
				</div>
			</div>
		</div>
        <div class="section">
	    	<div class="container">
				...
				<div class="col-sm-12">
<?php
$city_sql = "select distinct cityname, StateProvCode from retail_listing where ListingIsActive = 'y' AND trim(cityname) <> '' and StateProvCode = '".trim($q[0])."' order by cityname";
...
<i><h5>It's fast, simple and Free!</h5></i>
                ...
                <input type="hidden" name="property_type" value="r">
...
if ($city_count > 0)
{
 	...
 	if ($city_row['CityDesc']) echo "<h3>".trim($q[0])." Retail Market</h3>".$city_row['CityDesc']."<br />"; 
}	
...
if ($city_count > 0)
{
 	...
 	while ($city_row = mysqli_fetch_array($city_result))
 		echo "<a href=\"/retail-for-rent/United-States/".$city_row['StateProvCode']."/".$city_row['cityname']."\">".$city_row['cityname']."</a><br />"; 
...
$sql_prop = "select SpaceAvailable, RentalRateMin from retail_listing where (CityName = '".trim(str_replace("-", " ", $search))."' OR StateProvCode = '".trim($search)."')".($q[1] ? "and (PostalCode = '".trim($q[1])."' OR StateProvCode = '".trim($q[1])."') " : "")."AND ListingIsActive = 'y'";

//	$sql_prop = "select SpaceAvailable, RentalRateMin from retail_listing where CityName = '".trim($q[0])."' AND ListingIsActive = 'y'";
...
echo "<h3>".trim($q[0])." Retail Stats</h3>";
...
echo "<h3>Available Layouts</h3>";
	...
	$sql_prop = "select distinct PropertySubType, count(*) AS num from retail_listing where CityName = '".trim($q[0])."' AND ListingIsActive = 'y' GROUP BY PropertySubType";
...
if ($rec_count > 0 && $num_of_pages > 1)
{
	...
	echo ($page == 1 ? "" : "<li><a href=\"/retail-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").(($last != 0 && $last != 1) ? "/page$last" : "").($new_query_str ? "/?".$new_query_str : "")."\">Previous</a></li>");

	for ($i = 1; $i <= $num_of_pages; $i++)
	{
		if ($page == $i) echo "<li class=\"active\"><span>".$i."</span></li>";
		else echo "<li><a href=\"/retail-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").($i != 1 ? "/page$i" : "").($new_query_str ? "/?".$new_query_str : "")."\">$i</a></li>";
	}
	
	echo ($page == $num_of_pages ? "" : "<li><a href=\"/retail-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "")."/page$next".($new_query_str ? "/?".$new_query_str : "")."\">Next</a></li>");
}
?>
					</ul>
				</div>
