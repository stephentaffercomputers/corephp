...
$vicinity_sql_stmt  = "SELECT * FROM retail_listing WHERE ListingIsActive = 'y' AND
...
while ($vicinity_row = mysqli_fetch_array($vicinity_result)) 
{
	...
	$sidebar_content .='<table class="results-table"><tbody class="results-tbody"><tr><td rowspan="2" class="span3"><a href="/retail-for-rent/United-States/'.$vicinity_row["StateProvCode"].'/'.$vicinity_row['CityName'].'/'.$vicinity_row['ListingID'].'"> <img class="img-polaroid" id="img'.$vicinity_row['ListingID'].'" alt="'.$vicinity_row['StreetAddress'].'" src=\'';
	...
	$sidebar_content .='<td style="text-align: left; padding-left: 20px;"><span class="address_h4"><a href="/retail-for-rent/United-States/'.$vicinity_row["StateProvCode"].'/'.$vicinity_row['CityName'].'/'.$vicinity_row['ListingID'].'">'.$vicinity_row['StreetAddress'].'</a>'.($vicinity_row['suiteno'] ? '<br> '.$vicinity_row['suiteno'] : '').'</span><h4 class="top_rent">';

	if ($vicinity_row['RentalRateMin'] == "Negotiable") $thisRate = "Negotiable";
	else $thisRate = '$ '.number_format($vicinity_row['MonthlyRate']).'/mo';
	...
	$sidebar_content .= ($vicinity_row['RentalRateMin'] == "Negotiable") ? "Negotiable" : number_format($vicinity_row['MonthlyRate'] / ereg_replace("[^-0-9\.]", "", $vicinity_row['SpaceAvailableTotal']), 2);
