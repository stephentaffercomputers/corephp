...
$title = 'US Retail Space for Rent - 1000s of Listings';
$meta_desc = 'View available retail space for rent throughout the United States. Browse 1000s of listings to find the perfect space for you.';

$curr_url = $_SERVER["SERVER_NAME"].$_SERVER["REQUEST_URI"];	

if (isset($_GET["q"])) $q = explode(",", $_GET["q"]);

//if (sizeof($q) == 1 ) $title = array_search(trim(str_replace("-", " ", $q[0])), $states).' Retail Space for Rent - ';
if (sizeof($q) == 1 ) $title = $q[0].' Retail Space for Rent - ';
...
if (isset($_GET["id"]))
{
	...
	{ 
		...
		if (count($exp_str) > 1)
		{
			...
			$title = $cityname.' Retail Space for Rent ';
	...
	$search_sql_stmt = 'select * from retail_listing where '.$where_clause.';';
	try {
		...
		if ($row['ListingIsActive'] == 'n')
		{
			...
			header("Location: https://leaseretailspace.net/retail-for-rent/United-States/".$row['StateProvCode']."/".$row['CityName']);
		}

		$title = $row['CityName'].' Retail Space for Rent - ' ;//.' '.$row['StateProvCode'];
...
if ($ref_file == 'property_details'){ //indivudal listing
	...
	$meta_desc = $row['StreetAddress']." Offers ".($row['SpaceAvailableTotal'] ? number_format(preg_replace('/\D/', '', $row['SpaceAvailableTotal']))." sq. ft. of " : "")."retail space for rent in ".$row['CityName'].". View this property and all other retail spaces for lease in ".$row['CityName'];
}
elseif ($ref_file == 'all_cities'){
	$title = 'Retail Space for Rent in the US - View All Cities';
	$meta_desc = "Leaseretailspace.net has listings in hundreds of cities throughout the US. View all of our cities and find the right retail location for your business.";
}
elseif ($ref_file == 'all_states'){
	$title = 'Retail Space for Lease in the US - View All States';
	$meta_desc = "Leaseretailspace.net has listings in many states throughout the US. View all of our cities and find the right retail location for your business.";
}
elseif ($ref_file == 'searchresults'){
//	$title = array_search(trim(str_replace("-", " ", $q[0])), $states).' Retail Space for Rent - '.(isset($_GET{'page'}) ? "Listings Page ".$_GET{'page'} : "View Retail Listings");
//	$meta_desc = "View ".(isset($_GET{'page'}) ? "page ".$_GET{'page'}." of " : "")."available retail space listings in ".array_search(trim(str_replace("-", " ", $q[0])), $states).". From boutiques to restaurants to anchors, find great deals on retail spaces for rent and get a free property report.";
	$title = $q[0].' Retail Space for Rent - '.(isset($_GET{'page'}) ? "Listings Page ".$_GET{'page'} : "View Retail Listings");
	$meta_desc = "View ".(isset($_GET{'page'}) ? "page ".$_GET{'page'}." of " : "")."available retail space listings in ".$q[0].". From boutiques to restaurants to anchors, find great deals on retail spaces for rent and get a free property report.";

}
elseif ($ref_file == 'about'){
	$title = 'About LeaseRetailSpace.net - View 1000s of Retail Listings';
	$meta_desc = "Learn more about LeaseRetailSpace.net. We're one of the nations largest retail listing sites with retail spaces for rent and sale.";
}
elseif ($ref_file == 'contact'){
	$title = 'Contact Us - LeaseRetailSpace.net';
	$meta_desc = "Contact page for LeaseRetailSpace.net. We're one of the nations largest retail listing sites.";
}
?>
<!DOCTYPE html>
        ...
        <div class="mainmenu-wrapper">
	        <div class="container topcontainer">
		        <nav id="mainmenu" class="mainmenu">
					<ul>
						<li class="logo-wrapper"><a href="/"><h3>Lease Retail Space</h3></a> <!--<h4>Find Retail Space For Lease and For Sale</h4>!--></li>
						<li>
