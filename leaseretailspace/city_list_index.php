...
$sql_list_cities = 'select distinct cityname, StateProvCode, count(*) AS num from retail_listing where trim(cityname) <> "" AND ListingIsActive = \'y\' group by cityname order by cityname;';
...
{
	if ($row['num'] > 9 || $row['cityname'] == 'Long Beach')
...
if (!in_array($res['cityname'], $name)) {
  ...
  print '<li style="padding:5px; border:0px !important;" class="list-group-item mob_city"><a href="/retail-for-rent/United-States/'.$res['StateProvCode']."/".$res['cityname'].'">'.$res['cityname'].'</a></li>';
