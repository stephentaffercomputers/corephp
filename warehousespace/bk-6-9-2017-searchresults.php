...
<script src="https://maps.google.com/maps/api/js?v=3.3&amp;libraries=geometry&amp;sensor=false" type="text/javascript"></script>
<?php
...
if (sizeof($q) > 1 ) $search_sql_stmt = "select distinct * from warehouse_listing where ListingIsActive = 'y' AND CityName ='".trim($q[0])."' and PostalCode = '".trim($q[1])."'";
else {
	if ($q[0] == 'Arlington' || $q[0] == 'arlington') $search_sql_stmt = "select distinct * from warehouse_listing WHERE ListingIsActive = 'y' AND CityName = '".$q[0]."'";
	else 
		if (strlen($q[0]) == 2) 
		{
		 	$search_sql_stmt = "select distinct * from warehouse_listing WHERE ListingIsActive = 'y' AND StateProvCode like '%".$q[0]."%'";
		 	$statecode = $q[0];
		}
		else
		{ 
				...
				{
					$search = $states[$q[0]];
			   $search_sql_stmt = "select distinct * from warehouse_listing WHERE ListingIsActive = 'y' AND  StateProvCode ='". $search."'";
				}
				else
				{
					$search = $q[0];
			  $search_sql_stmt = "select distinct * from warehouse_listing WHERE ListingIsActive = 'y' AND concat(CityName, StateProvCode, PostalCode) like '%".trim(str_replace("-", " ", $search))."%'";
				}
		}
	...
	if ($rec_count > 0)
	{
		...
		while ($row = mysqli_fetch_array($result_page)) {
        	...
        	$sidebar_content .= '<div class="col-sm-4"><div class="shop-item"><div class="image">
								<a href="/warehouse-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'"><img class="img-polaroid" id="img'.$row['ListingID'].'" height="150" alt="'.$row['StreetAddress'].'" src="';
			...
			$sidebar_content .='"></a>
							...
							<!-- Product Title -->
							<div class="title">
								<h3><a href="/warehouse-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'">'.$mod_StreetAddress.'</a></h3><center><small>'.$row['CityName'].', '.$row["StateProvCode"].'</small></center>
							</div>
							<div class="price">';
...
if ($row['RentalRateMin'] != "Negotiable") $sidebar_content .=  '<br /><span style="font-weight: normal; font-size: 14px;">('.number_format(ereg_replace(",", "", $thisRate) / ereg_replace("[^-0-9\.]", "", $row['SpaceAvailable']), 2).' $/sf./month)</span>';
			...
			$sidebar_content .= '</div><div class="description">
							...
							<div class="actions">
								<a href="/warehouse-for-rent/United-States/'.$row["StateProvCode"].'/'.$row['CityName'].'/'.$row['ListingID'].'" class="btn">More Info</a>
							</div>
						</div></div>';
		}
...
-->
		...
		<div class="section section-breadcrumbs">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h1><a href="/">Home</a> / <? if ($q[0] != $cityname) echo $statecode; else echo "<a href=\"/warehouse-for-rent/United-States/".$statecode."\">".$statecode."</a> / ".$cityname; ?> Warehouses for Rent</h1>
					</div>
				</div>
			</div>
		</div>
...
if ($city_count > 0)
{
 	...
 	while ($city_row = mysqli_fetch_array($city_result))
 		echo "<a href=\"/warehouse-for-rent/United-States/".$city_row['StateProvCode']."/".$city_row['cityname']."\">".$city_row['cityname']."</a><br />"; 
...
$sql_prop = "select SpaceAvailable, RentalRateMin from warehouse_listing where (CityName = '".trim(str_replace("-", " ", $search))."' OR StateProvCode = '".trim($search)."')".($q[1] ? "and (PostalCode = '".trim($q[1])."' OR StateProvCode = '".trim($q[1])."') " : "")."AND ListingIsActive = 'y'";
...
if ($rec_count > 0 && $num_of_pages > 1)
{
	...
	echo ($page == 1 ? "" : "<li><a href=\"/warehouse-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").(($last != 0 && $last != 1) ? "/page$last" : "").($new_query_str ? "/?".$new_query_str : "")."\">Previous</a></li>");

	for ($i = 1; $i <= $num_of_pages; $i++)
	{
		if ($page == $i) echo "<li class=\"active\"><span>".$i."</span></li>";
		else echo "<li><a href=\"/warehouse-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").($i != 1 ? "/page$i" : "").($new_query_str ? "/?".$new_query_str : "")."\">$i</a></li>";
	}
	
	echo ($page == $num_of_pages ? "" : "<li><a href=\"/warehouse-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "")."/page$next".($new_query_str ? "/?".$new_query_str : "")."\">Next</a></li>");
}
?>
					</ul>
				</div>
