...
//echo $search_sql_stmt = "select * from warehouse_listing WHERE ListingIsActive = 'y' and ListingID='14551'";

echo $search_sql_stmt = "SELECT * FROM `warehouse_listing` WHERE ListingID BETWEEN 19000 and 19200";
...
$listing_id = $data['ListingID']."_".preg_replace("/[^a-zA-Z0-9]/", "_", $data['Title']);
			$image = file_get_contents('https://maps.googleapis.com/maps/api/streetview?size=640x640&location='.$data['Latitude'].','.$data['Longitude'].'&key=AIzaSyCNswMe5_HsaKaYjqmzPe-FP-19rBqafd4');

			$fp  = fopen('images/json_photos/'.$listing_id.".jpg", 'w+'); 
			fputs($fp, $image); 
$photo_url = "https://www.warehousespaces.com/images/json_photos/".$listing_id.".jpg";
        echo "<br>" .  $update_sql = "UPDATE warehouse_listing SET PhotoURL= '".$photo_url ."' where ListingID= '".$data['ListingID']."'";
            $result = $mysqli->query($update_sql);
}
