...
$search_sql_stmt = "SELECT *  FROM office_listing WHERE ListingIsActive = 'y' AND Latitude between ".$mapbounds->{"_southWest"}->{"lat"}." and ".$mapbounds->{"_northEast"}->{"lat"}." and Longitude between ".$mapbounds->{"_southWest"}->{"lng"}." and ".$mapbounds->{"_northEast"}->{"lng"}." ORDER BY ListingID;";
