...
$sql_list_cities = 'select distinct cityname, StateProvCode, count(*) AS num from office_listing where ListingIsActive = "y" AND trim(cityname) <> ""  group by cityname order by cityname;';
...
{
	if ($row['num'] > 20 || $row['cityname'] == 'Long Beach')
...
if (!in_array($res['cityname'], $name)) {
  ...
  print '<li style="padding:5px; border:0px !important;" class="list-group-item mob_city"><a href="/offices-for-rent/'.$res['StateProvCode']."/".$res['cityname'].'">'.$res['cityname'].'</a></li>';
