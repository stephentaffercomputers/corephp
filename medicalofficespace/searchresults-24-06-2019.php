...
<!--[if lte IE 8]>
<link rel="stylesheet" href="https://medicalofficespace.us/leaflet/leaflet.ie.css" />
<![endif]-->
<link rel="stylesheet" href="https://medicalofficespace.us/js/leaflet/MarkerCluster.Default.css" />
<!--[if lte IE 8]>
<link rel="stylesheet" href="https://medicalofficespace.us/leaflet/MarkerCluster.Default.ie.css" />
<![endif]-->
...
<!--<script src="https://maps.google.com/maps/api/js?v=3.3&amp;libraries=places&amp;sensor=false" type="text/javascript"></script>!-->
<?php
...
if (sizeof($q) > 1 ) $search_sql_stmt = "select distinct *, 1 AS rank from medical_listing where  ListingIsActive = 'y' AND CityName ='".trim($q[0])."' and PostalCode = '".trim($q[1])."'";
	...
	if ($q[0] == 'Arlington' || $q[0] == 'arlington') $search_sql_stmt = "select distinct *, 1 AS rank from medical_listing WHERE  ListingIsActive = 'y' AND CityName = '".$q[0]."'";
		...
		if (strlen($q[0]) == 2) $search_sql_stmt = "select distinct *, 1 AS rank from medical_listing WHERE ListingIsActive = 'y' AND StateProvCode like '%".$q[0]."%'";
		else 
		{
		//	$search_sql_stmt = "select distinct * from medical_listing WHERE ListingIsActive = 'y' AND concat(CityName, StateProvCode, PostalCode) like '%".$q[0]."%'";
 				if(!empty($states[$q[0]]))
				{
					$search = $states[$q[0]];
...
try {
	...
	if ($rec_count > 0)
	{
		...
		while ($row = mysqli_fetch_array($result_page)) {
        	...
        	//////////////////////////////////////////////
if ($row['RentalRateMin'] == "Negotiable") $sidebar_content .=  "";
else
{
	if (strpos($row['RentalRateMin'], 'Year') !== false)
	{
     ...
     $thisRate = number_format(floatval(ereg_replace("[^-0-9\.]", "", $row['RentalRateMin'])/12));
	}
...
$price_check =  number_format(ereg_replace(",", "", $thisRate) / ereg_replace("[^-0-9\.]", "", $row['SpaceAvailable']), 2); 
if($price_check >= 1.50 && $row['RentalRateMin'] != "Negotiable") {
        	$sidebar_content .= '<div class="col-sm-4"><div class="shop-item"><div class="image">
								<a href="/medical-office-for-rent/United-States/'.$row["StateProvCode"].'/'.ltrim($row['CityName']).'/'.$row['ListingID'].'"><img class="img-polaroid" id="img'.$row['ListingID'].'" height="150" alt="'.$row['StreetAddress'].'" src="';
			...
			$sidebar_content .='"></a>
							...
							<!-- Product Title -->
							<div class="title">
								<h3><a href="/medical-office-for-rent/United-States/'.$row["StateProvCode"].'/'.ltrim($row['CityName']).'/'.$row['ListingID'].'">'.$mod_StreetAddress.'</a></h3><center><small>'.$row['CityName'].', '.$row["StateProvCode"].'</small></center>
							</div>
							<div class="price">';
...
if ($row['RentalRateMin'] != "Negotiable") $sidebar_content .=  '<br /><span style="font-weight: normal; font-size: 14px;">($'.number_format(ereg_replace(",", "", $thisRate) / ereg_replace("[^-0-9\.]", "", $row['SpaceAvailable']), 2).'/sf./month)</span>';
			...
			$sidebar_content .= '</div><div class="description">
							...
							<div class="actions">
								<a href="/medical-office-for-rent/United-States/'.$row["StateProvCode"].'/'.ltrim($row['CityName']).'/'.$row['ListingID'].'" class="btn">More Info</a>
							</div>
						</div></div>';
}
...
if ($row['RentalRateMin'] != "Negotiable") $sidebar_content .=  '<br /><span style="font-weight: normal; font-size: 14px;">($'.number_format(ereg_replace(",", "", $thisRate) / ereg_replace("[^-0-9\.]", "", $row['SpaceAvailable']), 2).'/sf./month)</span>';
		...
		while ($row = mysqli_fetch_array($result)) {        
    		if ($row["Latitude"] != "")	{
				...
				" var contentTxt = '<div><div class=\"row-fluid\"><a href=\"/medical-office-for-rent/United-States/".$row['StateProvCode']."/".$row['CityName']."/".$row['ListingID']."\">".preg_replace($patterns, '', $row['StreetAddress'].", ".ltrim($row['CityName']))."</a><br />".$row['SpaceAvailableTotal']." sq. ft.</div></div>';".
	...
	else
	{
		...
		//$sidebar_content .= "<strong>Your search did not match any properties.</strong><br /><br /><a href=\"https://medicalofficespace.us\">Start a New Search</a>"; 
...
-->
		...
		<div class="section section-breadcrumbs">
			<div class="container">
				<div class="row">
					<div class="col-md-12">
						<h1><a href="/">Home</a> / <? if ($q[0] != $cityname) echo $statecode; else echo "<a href=\"/medical-office-for-rent/United-States/".$statecode."\">".$statecode."</a> / ".$cityname; ?> Medical Office Spaces for Rent</h1>
					</div>
				</div>
			</div>
		</div>
        <div class="section">
	    	<div class="container">
				...
				<div class="col-sm-12">
					<!-- Sidebar -->
<?php
$city_sql = "select distinct cityname, StateProvCode from medical_listing where trim(cityname) <> '' and StateProvCode = '".trim($q[0])."' order by cityname";
...
-->	
						...
						<div class="row-fluid">
							<form class="form-horizontal" role="form" id="filter-form" method="get" action="<? echo $_SERVER['REQUEST_URI']; ?>">
								<!--<center><strong>Filter Results by: </strong></center>-->
								...
								<!--<div class="form-group col-sm-3">
		        				 	<label for="space_type" class="col-sm-12 control-label"><b>Space Type:</b></label>
		        				 	<div class="col-sm-12">
										<? list_prop_types(); ?>	
									</div>
								</div>-->
...
<?php 	echo "<h3>".trim($q[0])." Hospitals</h3>"; ?>
...
if ($city_count > 0)
{
 	...
 	while ($city_row = mysqli_fetch_array($city_result))
 		echo "<a href=\"/medical-office-for-rent/United-States/".$city_row['StateProvCode']."/".$city_row['cityname']."\">".$city_row['cityname']."</a><br />"; 
 	echo "<br />";
}
...
$sql_prop = "select SpaceAvailable, RentalRateMin from medical_listing where (CityName = '".trim(str_replace("-", " ", $search))."' OR StateProvCode = '".trim($search)."')".($q[1] ? "and (PostalCode = '".trim($q[1])."' OR StateProvCode = '".trim($q[1])."') " : "")."AND ListingIsActive = 'y'";
...
echo "<h3>".trim($q[0])." Medical Office Stats</h3>";
...
if ($rec_count > 0 && $num_of_pages > 1)
{
	...
	echo ($page == 1 ? "" : "<li><a href=\"/medical-office-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").(($last != 0 && $last != 1) ? "/page$last" : "").($new_query_str ? "/?".$new_query_str : "")."\">Previous</a></li>");

	for ($i = 1; $i <= $num_of_pages; $i++)
	{
		if ($page == $i) echo "<li class=\"active\"><span>".$i."</span></li>";
		else echo "<li><a href=\"/medical-office-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "").($i != 1 ? "/page$i" : "").($new_query_str ? "/?".$new_query_str : "")."\">$i</a></li>";
	}
	
	echo ($page == $num_of_pages ? "" : "<li><a href=\"/medical-office-for-rent/United-States/$statecode".($q[0] == $cityname ? "/$cityname" : "")."/page$next".($new_query_str ? "/?".$new_query_str : "")."\">Next</a></li>");
}
?>
					</ul>
				</div>
