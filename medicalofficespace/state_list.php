...
$sql_list_cities = 'select distinct StateProvCode from medical_listing where ListingIsActive = \'y\' AND trim(StateProvCode) <> "" order by StateProvCode;';
...
{
	$num = 0;
	if(!empty($states[$row['StateProvCode']])) {

	print "<li><a href=\"/medical-space/United-States/".$row['StateProvCode']."\">".ltrim($states[$row['StateProvCode']])."</a><br />Cities Covered: ";
	
	$sql = 'select distinct cityname from medical_listing where ListingIsActive = \'y\' AND StateProvCode = "'.$row['StateProvCode'].'"';
