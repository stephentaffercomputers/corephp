<?php
$sql_list_cities = 'select distinct cityname, StateProvCode,PropertyType from listings where PropertyType="Industrial"  AND ListingIsActive = \'y\' AND trim(cityname) <> "" order by cityname;';
...
{	
	print "<li><a href=\"/united-states/".$row['StateProvCode']."/".$row['cityname']. "/". $prop_type_array[strtolower($row['PropertyType'])]."\">".$row['cityname'].($row['cityname'] == "Newark" ? " ".$row['StateProvCode'] : "")." Warehouse Space</a></li>\n";
