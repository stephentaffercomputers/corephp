		...
		<div class="section section-breadcrumbs">
			<div class="container">
				<div class="row">
					...
					<div class="col-md-9">
						<h1><a href="/">Home</a> / <a href="/commercial-real-estate/United-States/<?=$row['StateProvCode']?>"><?=$row['StateProvCode']?></a> / <a href="/commercial-real-estate/United-States/<?=$row['StateProvCode']?>/<?=$row['CityName']?>"><?=$row['CityName']?> Commercial Real Estate</a> / <a href="/united-states/<?=$row['StateProvCode']?>/<?=$row['CityName']?>/<?= ($row['PropertyType'] == "Industrial" ? "warehouse" : strtolower($row['PropertyType'])) ?>-space/"><?= ($row['PropertyType'] == "Industrial" ? "Warehouse" : $row['PropertyType']) ?> for Rent</a></h1>
					</div>
...
$sql = "SELECT count(*) AS total_num_listings FROM listings WHERE ListingIsActive = 'y' AND CityName = '".$row['CityName']."' AND StateProvCode = '".$row['StateProvCode']."' AND PropertyType = '".$row['PropertyType']."'".($row['BuildingClass'] ? " AND BuildingClass = '".$row['BuildingClass']."'" : "");
...
try {
	...
	$total_num_listings = $stat['total_num_listings'];
}
catch(exception $e) {var_dump($e);}

if ($total_num_listings > 1)
	$stat_desc = "One of ".$total_num_listings." ".($row['BuildingClass'] ? "Class ".$row['BuildingClass'] : "")." spaces in ".$row['CityName'];
else
	$stat_desc = "The only ".($row['BuildingClass'] ? "Class ".$row['BuildingClass'] : "")." space on the market in ".$row['CityName'];
	
$stat_desc .= ", the asking rate for this ".strtolower($row['PropertyType'])." space is ".($thisRate == "Inquire about rate" ? "negotiable" : $thisRate);

if ($thisRate != "Inquire about rate")
{
	$sql = "SELECT count(*) AS total_listings FROM listings WHERE ListingIsActive = 'y' AND CityName = '".$row['CityName']."' AND StateProvCode = '".$row['StateProvCode']."' AND PropertyType = '".$row['PropertyType']."'";
	...
	$sql = "SELECT count(*) AS listings_with_lower_rate FROM listings WHERE ListingIsActive = 'y' AND CityName = '".$row['CityName']."' AND StateProvCode = '".$row['StateProvCode']."' AND PropertyType = '".$row['PropertyType']."' AND MonthlyRate < ".$row['MonthlyRate'];
	...
	try {
		...
		$listings_with_lower_rate = $stat['listings_with_lower_rate'];
	}
	catch(exception $e) {var_dump($e);}
	
	if ($total_listings > 0) $stat_desc .= ", ranking it #".($listings_with_lower_rate + 1)." of ".$total_listings."  properties";

	if ($row['BuildingClass'])
	{
		$sql = "SELECT count(*) AS same_class_listings_with_lower_rate FROM listings WHERE ListingIsActive = 'y' AND CityName = '".$row['CityName']."' AND StateProvCode = '".$row['StateProvCode']."' AND PropertyType = '".$row['PropertyType']."' AND BuildingClass = '".$row['BuildingClass']."' AND MonthlyRate < ".$row['MonthlyRate'];
		...
		try {
			...
			$same_class_listings_with_lower_rate = $stat['same_class_listings_with_lower_rate'];
		}
		catch(exception $e) {var_dump($e);}
		
		$stat_desc .= " and #".($same_class_listings_with_lower_rate + 1)." of ".$total_num_listings." Class ".$row['BuildingClass']." properties";
	}
}

$stat_desc .= ".";

$desc_parts = explode("Note: ", $row["Description"]);

$row["Description"] = $desc_parts[0].$stat_desc." Note: ".$desc_parts[1];
?>
</small></h1> 
        			</div>
...
<table>
	    			...
	    			<div class="col-sm-4 product-details" style="box-shadow: 0px 1px #FFF inset, 0px 0px 8px #C8CFE6;">	    			    
...
<p>&nbsp;</p>
<div id="map_canvas" style="float: left; height: 400px; width: 100%; margin: 30px 0;"></div>
	    				<div id="pano" style="float: left; height: 400px; width: 100%; margin-bottom: 30px;"></div>
	    			</div>
