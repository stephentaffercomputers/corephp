...
$link = mysqli_connect("localhost", "searchco_db", "7T2kHlRhuyLHFEKU", "searchco_db");

//$link = mysqli_connect("localhost", "warehous_root", "7Pt2iFKOy8WYeCwOh", "warehous_main");
$Path = file('https://searchcommercialspace.com/dev/xaj.jsonlines');
...
//    $date = strototime(date('Y-m-d H:i:s'));
    ...
    if(isset($Output->address) && !empty($Output->address))
    {
        ...
        $select_query = mysqli_query($link, "SELECT * FROM listings_new where Title='".$Output->address."'");
        if(mysqli_num_rows($select_query) > 0){
        //echo 'test'; exit;
        echo "<br>" . $update_sql = "UPDATE listings_new SET Title = '".$title."', SpaceNumber =  '".$space_number."', SpaceAvailable = '".$space_available."', RentalRate = '".$RentalRate."', MonthlyRate = '', StreetAddress = '".$street_address."', CityName = '".$city_name."', StateProvCode = '".$state_code."', PostalCode = '".$postal_code."', LotSize = '', LastUpdate = '', SpaceAvailableMax = '".$space_available."', SpaceAvailableMin = '".$space_available."', PropertySubType = 'Industrial', PropertyType = '".$property_type."', RentalRateMin = '".$price."', SpaceAvailableTotal = '".$space_available."', Description = '".$description."', Latitude = '".$latitude."', Longitude = '".$longitude."', PhotoURL = '', PhotoURL2 = '', PhotoURL3 = '', PhotoURL4 = '', PhotoURL5 = '',ListingIsActive = 'y', AgentName = '', AgentEmail = '', AgentPhone = '".$agent_phone."', created_from = 'json' where Title = '".$title."'";
